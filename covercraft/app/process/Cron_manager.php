...
class Ssh2_crontab_manager
{
    ...
    function __construct($host = NULL, $port = NULL, $username = NULL, $password = NULL)
    {
        ...
        try
        {
            if(is_null($host) || is_null($port) || is_null($username) || is_null($password)) throw new Exception("Please specify host, port, username and password");
    ...
    public function write_to_file($path = NULL, $handle = NULL)
    {
        if( ! $this->crontab_file_exists())
        {
            ...
            $init_cron = "crontab -1 > {$this->cron_file} && [ -f {$this->cron_file} ] || > {$this->cron_file}";
    ...
    {
        if(is_null($cron_jobs)) $this->error_messsage("Nothing to append! Please specify a process job or an array of process jobs.");
    ...
    {
        if (is_null($cron_jobs)) $this->error_message("Nothing to remove!  Please specify a process job or an array of process jobs.");
    ...
    private function crontab_file_exits()
    {
